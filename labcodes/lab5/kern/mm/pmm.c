...
//#if 0
    ...
    if ( (*pdep & PTE_P) == 0) {            // (2) check if entry is not present
        ...
        if (create) p = alloc_page();else return NULL;       // CAUTION: this page is used for page table, not for common data page
...
//#if 0
    ...
    if (* ptep & PTE_P) {                                        //(1) check if this page table entry is present
        ...
        int now_ref;
        now_ref = page_ref_dec(page);                            //(3) decrease page reference
        if (now_ref == 0) free_page(page);                       //(4) and free this page when page reference reachs 0
